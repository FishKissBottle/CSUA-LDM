...
VAE_CODE_ROOT = VARIANT_ROOT / "CSUA_LDM_VAE_NoDisent" / "CSUA_LDM_VAE_NoDisent_Code"
DIFFUSION_CODE_ROOT = VARIANT_ROOT / "CSUA_LDM_Diffusion_NoDisent" / "CSUA_LDM_Diffusion_NoDisent_Code"
...
import CSUA_LDM_Diffusion_NoDisent_sampler as sampler_mod
...
from CSUA_LDM_VAE_NoDisent_helpers import build_vae_model, reproject_images_to_latent_mu
from CSUA_LDM_VAE_NoDisent_model import LatentAutoEncoderNoDisent


CSUA_LDM_EVAL_TAG = "CSUA_LDM_NoDisent"
CSUA_LDM_EVAL_UNCERTAINTY_GUIDED_TAG = "CSUA_LDM_NoDisent_UncGuided"
CSUA_LDM_EVAL_OUTPUT_ROOT = build_standard_results_root("spectral_evaluate", CSUA_LDM_EVAL_TAG)
CSUA_LDM_EVAL_UNCERTAINTY_GUIDED_OUTPUT_ROOT = build_standard_results_root("spectral_evaluate", CSUA_LDM_EVAL_UNCERTAINTY_GUIDED_TAG)
...
def run_diffusion_chain(
    ...
    """Run NoDisent inference from lr_up through joint-latent diffusion."""
    lr_up_img = ensure_batched_4d(lr_up_img).to(device)
    _, _, z, logvar = reproject_images_to_latent_mu(
        imgs=lr_up_img,
        model=vae_model,
        device=device,
    )
    denoise_out = CSUA_LDM_Diffusion_NoDisent_Denoise(
        y0_e=z.to(device),
        y0_unc_e=torch.exp(logvar).to(device),
        x0_gt_e=z.to(device),
    ...
    chain_out = {"latents": {"y0": z.to(device)}}
    if return_uncertainty:
        chain_out["images"] = {"hr": denoise_out["sr_mean"]}
        chain_out["latents"]["hr"] = denoise_out["latent_mean"]
        chain_out["reverse_uncertainty"] = {
            "hr": {
                "var": denoise_out["reverse_uncert_var"],
                "std": denoise_out["reverse_uncert_std"],
            }
        }
        chain_out["latent_uncertainty"] = {
            "hr": {
                "var": denoise_out["latent_uncert_var"],
                "std": denoise_out["latent_uncert_std"],
            }
        }
        chain_out["pixel_uncertainty"] = {
            "hr": {
        ...
        pred_img, full_latent, _latent_logvar = denoise_out
        chain_out["images"] = {"hr": pred_img}
        chain_out["latents"]["hr"] = full_latent
    return chain_out


def build_diffusion_model(device: torch.device) -> tuple[CSUA_LDM_Diffusion_NoDisent_UNet, bool, int]:
    model = CSUA_LDM_Diffusion_NoDisent_UNet(
    ...
    return load_model_with_ema_fallback(model, CSUA_LDM_DIFFUSION_NODISENT_MODEL_SAVEPATH)


def configure_sampling_uncertainty_guidance(mode: str) -> bool:
    mode = str(mode).strip().lower()
    if mode == "yaml":
        enabled = bool(CSUA_LDM_DIFFUSION_NODISENT_INFERENCE_UNCERT_GUIDANCE_ENABLED)
    elif mode == "on":
        enabled = True
    elif mode == "off":
        enabled = False
    ...
    sampler_mod.CSUA_LDM_DIFFUSION_NODISENT_INFERENCE_UNCERT_GUIDANCE_ENABLED = enabled
    return enabled


def main():
    parser = argparse.ArgumentParser(description="Spectrally evaluate CSUA_LDM_NoDisent with configurable sampling uncertainty guidance.")
    parser.add_argument(
    ...
    artifact_stem = Path(CSUA_LDM_DIFFUSION_NODISENT_MODEL_SAVEPATH).stem
    ...
    vae_model, vae_loaded_ema, vae_checkpoint_epoch = build_vae_model(
        Path(CSUA_LDM_VAE_NODISENT_SAVEPATH),
        device,
        load_ema=True,
    )
    ...
    write_spectral_evaluation_info(
        ...
        diffusion_checkpoint_path=CSUA_LDM_DIFFUSION_NODISENT_MODEL_SAVEPATH,
        diffusion_checkpoint_epoch=diffusion_checkpoint_epoch,
        diffusion_loaded_ema=diffusion_loaded_ema,
        diffusion_model=diffusion_model,
        vae_checkpoint_path=CSUA_LDM_VAE_NODISENT_SAVEPATH,
