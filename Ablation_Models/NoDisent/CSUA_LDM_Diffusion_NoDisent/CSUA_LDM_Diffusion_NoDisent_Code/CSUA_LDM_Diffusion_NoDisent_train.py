...
VAE_ROOT = VARIANT_ROOT / "CSUA_LDM_VAE_NoDisent"
VAE_CODE_ROOT = VAE_ROOT / "CSUA_LDM_VAE_NoDisent_Code"
...
from CSUA_LDM_NoDisent_Config import *  # type: ignore[reportMissingImports]
...
from CSUA_LDM_Diffusion_NoDisent_helpers import (
    cosine_beta_scheduler,
    select_dual_branch_source,
)
from CSUA_LDM_Diffusion_NoDisent_model import CSUA_LDM_Diffusion_NoDisent_UNet
from CSUA_LDM_Diffusion_NoDisent_denoise import CSUA_LDM_Diffusion_NoDisent_Denoise
...
from CSUA_LDM_VAE_NoDisent_helpers import reproject_images_to_latent_mu
from CSUA_LDM_VAE_NoDisent_model import LatentAutoEncoderNoDisent

gdal.UseExceptions()

NODISENT_LOSS_METRIC_KEYS = (
    "total",
    "gnll",
    "v_l1",
    "v_l2",
...
class DiffusionLossCalculator(nn.Module):
    ...
    def __init__(
        ...
        model,
        betas: torch.Tensor,
        lambda_gnll: float = CSUA_LDM_DIFFUSION_NODISENT_GNLL_WEIGHT,
        lambda_l1: float = CSUA_LDM_DIFFUSION_NODISENT_V_L1_WEIGHT,
        lambda_l2: float = CSUA_LDM_DIFFUSION_NODISENT_V_L2_WEIGHT,
        lambda_confidence: float = CSUA_LDM_DIFFUSION_NODISENT_V_CONFIDENCE_WEIGHT,
...
def _encode_image_to_latent(
    ...
    with torch.amp.autocast("cuda", enabled=False):
        ...
        z, mu, logvar = vae_model.reparameterize(z_params)
    if bool(use_mu):
        return mu, logvar
    return z, logvar
...
def _decode_latent_to_mean_and_var(
    ...
    vae_dtype = _get_module_param_dtype(vae_model)
    z_in = z.to(device=z.device, dtype=vae_dtype)
    logvar_in = logvar.to(device=logvar.device, dtype=vae_dtype)
    with torch.amp.autocast("cuda", enabled=torch.cuda.is_available(), dtype=torch.bfloat16):
        dec_out = vae_model.decode(z_in, logvar_in)
    ...
    return pred_mean.to(dtype=z.dtype), pred_var.to(dtype=z.dtype)
...
def _compute_loss_for_step(
    ...
    Latent_model: LatentAutoEncoderNoDisent,
    loss_calculator: DiffusionLossCalculator,
    use_amp: bool,
    source_key: str = "lr_up",
):
    device = source_imgs.device
    use_reproj_source = str(source_key).strip().lower() == "lr_up"

    with torch.no_grad():
        if use_reproj_source:
            (
                _reproj_mean,
                _reproj_pixel_var,
                cond_z,
                cond_logvar,
            ) = reproject_images_to_latent_mu(
                source_imgs,
                Latent_model,
                draw_microbatch_size=CSUA_LDM_DIFFUSION_NODISENT_DRAW_MICROBATCH_SIZE,
                device=device,
            )
        else:
            cond_z, cond_logvar = _encode_image_to_latent(
                Latent_model,
                source_imgs,
                use_mu=False,
            )
        target_z, target_logvar = _encode_image_to_latent(
            Latent_model,
            target_imgs,
            use_mu=False,
        )
    ...
    if use_amp and torch.cuda.is_available():
        with torch.amp.autocast("cuda", dtype=torch.bfloat16):
            loss_dict = loss_calculator.compute_loss_dict(
                cond_z,
                target_z,
                cond_logvar_content=cond_logvar,
            )
    else:
        loss_dict = loss_calculator.compute_loss_dict(
            cond_z,
    ...
    loss_gnll = loss_dict["loss_gnll"]
    loss_v_l1 = loss_dict["loss_v_l1"]
    loss_v_l2 = loss_dict["loss_v_l2"]
    loss_conf = loss_dict["loss_v_conf"]
    loss_metrics = {
        "total": loss,
        "gnll": loss_gnll,
        "v_l1": loss_v_l1,
...
class OneStepSREvaluator:
    """Evaluate one-step SR for the NoDisent ablation (w/o disentangled latent design)."""

    def __init__(
        self,
        vae_model: LatentAutoEncoderNoDisent,
        diffusion_model: CSUA_LDM_Diffusion_NoDisent_UNet,
        hr_mean: torch.Tensor,
        hr_std: torch.Tensor,
        draw_microbatch_size: int = CSUA_LDM_DIFFUSION_NODISENT_DRAW_MICROBATCH_SIZE,
    ):
    ...
    def _prepare_inputs(
        ...
        for key in self._required_target_keys:
            if key not in sr_targets:
                raise KeyError(f"Missing SR target: {key}.")
            target_imgs = ensure_batched_4d(sr_targets[key]).to(device=device, non_blocking=True)
            sr_targets[key] = target_imgs
    ...
    def _process_microbatch(
        ...
        chunk_size = int(lr_up_chunk.shape[0])
        ...
        target_img_name = "hr"
        prev_target_imgs = sr_targets[target_img_name][batch_slice]
        prev_target_z, prev_target_logvar = _encode_image_to_latent(
            self.vae_model, prev_target_imgs, use_mu=False,
        )
        prev_target_mu, _ = _encode_image_to_latent(
            self.vae_model, prev_target_imgs, use_mu=True,
        )

        y0_unc_e = torch.exp(cond_logvar)

        if return_uncertainty:
            denoise_out = CSUA_LDM_Diffusion_NoDisent_Denoise(
                y0_e=cond_z,
                y0_unc_e=y0_unc_e,
                x0_gt_e=prev_target_z,
                x0_gt_mu_e=prev_target_mu,
                init_seed=0,
                steps=CSUA_LDM_DIFFUSION_NODISENT_STEPS,
            ...
            sr_prev_imgs_e = denoise_out["latent_mean"]
            self._reverse_unc_chunks[target_img_name].append(denoise_out["reverse_uncert_std"])
            self._latent_unc_chunks[target_img_name].append(denoise_out["latent_uncert_std"])
            self._pixel_unc_chunks[target_img_name].append(denoise_out["pixel_uncert_std"])
        else:
            sr_prev_imgs, sr_prev_imgs_e, latent_logvar_out = CSUA_LDM_Diffusion_NoDisent_Denoise(
                y0_e=cond_z,
                y0_unc_e=y0_unc_e,
                x0_gt_e=prev_target_z,
                x0_gt_mu_e=prev_target_mu,
        ...
        self._image_chunks[target_img_name].append(sr_prev_imgs)
...
def train(data_loader, Latent_model, optimizer, loss_calculator,
         gnll_weight: float = CSUA_LDM_DIFFUSION_NODISENT_GNLL_WEIGHT,
         l1_weight: float = CSUA_LDM_DIFFUSION_NODISENT_V_L1_WEIGHT,
         l2_weight: float = CSUA_LDM_DIFFUSION_NODISENT_V_L2_WEIGHT,
         confidence_weight: float = CSUA_LDM_DIFFUSION_NODISENT_V_CONFIDENCE_WEIGHT,
         grad_clip_max_norm: float | None = CSUA_LDM_GRAD_CLIP_MAX_NORM,
         epoch_idx: int = 0,
         ema_model=None,
         ema_decay=0.995,
         train_microbatch_size: int = CSUA_LDM_DIFFUSION_NODISENT_TRAIN_MICROBATCH_SIZE,
         start_global_iteration: int = 0,
         max_iterations: int = 0,):
    """
    Train loop for CSUA_LDM diffusion (NoDisent: w/o disentangled latent design).
    ...
    metric_sums = _init_metric_sums(NODISENT_LOSS_METRIC_KEYS)
    num_samples = 0
    ...
    for batch in pbar:
        ...
        batch_metric_sums = _init_metric_sums(NODISENT_LOSS_METRIC_KEYS)
        skip_batch = False
        loss_terms = None
        weighted_loss = None
        loss = None
        ...
        for mb_start, mb_end in iter_microbatch_slices(batch_size, train_microbatch_size):
            ...
            source_imgs_mb = source_imgs[mb_start:mb_end]
            target_imgs_mb = hr_imgs[mb_start:mb_end]

            loss_terms = _compute_loss_for_step(
                source_imgs=source_imgs_mb,
                target_imgs=target_imgs_mb,
            ...
            for key in NODISENT_LOSS_METRIC_KEYS:
...
def valid(data_loader, Latent_model, loss_calculator,
         ...
         epoch_idx: int = 0,
         eval_microbatch_size: int = CSUA_LDM_DIFFUSION_NODISENT_VALID_MICROBATCH_SIZE):
...
def ensure_output_dirs():
    os.makedirs(os.path.dirname(CSUA_LDM_DIFFUSION_NODISENT_MODEL_SAVEPATH), exist_ok=True)
    os.makedirs(CSUA_LDM_DIFFUSION_NODISENT_LOG_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_DIFFUSION_NODISENT_RGB_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_DIFFUSION_NODISENT_TIF_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_DIFFUSION_NODISENT_UNC_RGB_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_DIFFUSION_NODISENT_UNC_TIF_DIR, exist_ok=True)
...
def main(dataset_dict: dict = CSUA_LDM_DATASET_DIRS,
         ...
         save_reverse_and_latent_uncertainty: bool = CSUA_LDM_DIFFUSION_NODISENT_SAVE_REVERSE_AND_LATENT_UNCERTAINTY,
         train_microbatch_size: int = CSUA_LDM_DIFFUSION_NODISENT_TRAIN_MICROBATCH_SIZE,
         eval_microbatch_size: int = CSUA_LDM_DIFFUSION_NODISENT_VALID_MICROBATCH_SIZE,
         draw_microbatch_size: int = CSUA_LDM_DIFFUSION_NODISENT_DRAW_MICROBATCH_SIZE):
    ...
    writer = SummaryWriter(CSUA_LDM_DIFFUSION_NODISENT_LOG_DIR)
    ...
    # ---------- safety ----------
    DIFFUSION_NODISENT_CHECKPOINT_PATH = CSUA_LDM_DIFFUSION_NODISENT_MODEL_SAVEPATH
    is_exist = os.path.exists(DIFFUSION_NODISENT_CHECKPOINT_PATH)
    if is_exist and not resume_train:
        raise Exception(f"{CSUA_LDM_DIFFUSION_NODISENT_TAG} checkpoint exists and resume_train is False.")

    # ---------- build diffusion model ----------
    csua_diffusion_model = CSUA_LDM_Diffusion_NoDisent_UNet(
        ch=CSUA_LDM_DIFFUSION_NODISENT_UNET_CH,
        out_ch=CSUA_LDM_LATENT_HIDDENCHANNEL,
        ch_mult=CSUA_LDM_DIFFUSION_NODISENT_UNET_CH_MULT,
        attn_resolutions=CSUA_LDM_DIFFUSION_NODISENT_UNET_ATTN_RESOLUTIONS,
        dropout=CSUA_LDM_DIFFUSION_NODISENT_UNET_DROPOUT,
        resamp_with_conv=CSUA_LDM_DIFFUSION_NODISENT_UNET_RESAMP_WITH_CONV,
        in_channels=CSUA_LDM_LATENT_HIDDENCHANNEL,
        resolution=CSUA_LDM_DIFFUSION_NODISENT_UNET_RESOLUTION,
        cond_channels=CSUA_LDM_LATENT_HIDDENCHANNEL,
        cond_scale=CSUA_LDM_DIFFUSION_NODISENT_COND_SCALE,
        window_attn_min_resolution=CSUA_LDM_DIFFUSION_NODISENT_WINDOW_ATTN_MIN_RESOLUTION,
        attn_base_head_dim=CSUA_LDM_DIFFUSION_NODISENT_ATTN_BASE_HEAD_DIM,
        attn_heads_cap=CSUA_LDM_DIFFUSION_NODISENT_ATTN_HEADS_CAP,
    ).to(DEVICE)
    ema_model = build_ema_model(csua_diffusion_model)

    optimizer = torch.optim.Adam(
        list(csua_diffusion_model.parameters()),
        lr=CSUA_LDM_LEARNING_RATE,
        betas=(0.9, 0.999),
    )
    # ---------- Diffusion schedule (betas) ----------
    betas = cosine_beta_scheduler(
        timesteps=CSUA_LDM_DIFFUSION_NODISENT_STEPS,
        device=DEVICE,
        dtype=torch.float32,
    )
    loss_calculator = DiffusionLossCalculator(
    ...
    if resume_train:
        csua_diffusion_model, optimizer, scheduler, last_epoch, last_loss_dict, ckpt_extra, loaded_ema_decay = load_model(
            DIFFUSION_NODISENT_CHECKPOINT_PATH,
            csua_diffusion_model,
            optimizer,
            scheduler,
            ema_model=ema_model,
    ...
    print(
        f"[{CSUA_LDM_DIFFUSION_NODISENT_TAG}] VRAM-friendly batch sizes => "
        f"train={CSUA_LDM_TRAIN_BATCH_SIZE}, valid={CSUA_LDM_VALID_BATCH_SIZE}, draw={CSUA_LDM_DRAW_BATCH_SIZE}"
    )
    print(
        f"[{CSUA_LDM_DIFFUSION_NODISENT_TAG}] Internal microbatch sizes => "
        f"train={train_microbatch_size}, valid={eval_microbatch_size}, draw={draw_microbatch_size}"
    )
    print(
        f"[{CSUA_LDM_DIFFUSION_NODISENT_TAG}] Dataset splits => "
        f"train={train_split_label}, valid={valid_split_label}, draw={draw_split_label}"
    )
    print(f"[{CSUA_LDM_DIFFUSION_NODISENT_TAG}] Using fixed one-step diffusion path: cond -> hr")

    # ---------- load latent VAE (once) ----------
    VAE_NODISENT_CHECKPOINT_PATH = CSUA_LDM_VAE_NODISENT_SAVEPATH
    if not os.path.exists(VAE_NODISENT_CHECKPOINT_PATH):
        raise Exception(f"{VAE_NODISENT_CHECKPOINT_PATH} does not exist")
    ...
    for epoch_idx in range(start_epoch, TRAIN_MAX_EPOCHS):
        ...
        print(
            ...
            f'V_L2_Loss: {valid_v_l2_loss:.6f}, Conf_Loss: {valid_conf_loss:.6f}\n'
            f'    Valid_Mode: one_step (cond -> hr)'
        )

        info_path = CSUA_LDM_DIFFUSION_NODISENT_MODEL_SAVEPATH.replace(".pth", "_TrainingInfo.txt")
        log_info(
            log_path=info_path,
        ...
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/train_loss', train_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/valid_loss',  valid_loss,  epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/train_gnll', train_gnll_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/valid_gnll', valid_gnll_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/train_l1', train_v_l1_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/valid_l1', valid_v_l1_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/train_l2', train_v_l2_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/valid_l2', valid_v_l2_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/train_v_l1', train_v_l1_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/valid_v_l1', valid_v_l1_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/train_v_l2', train_v_l2_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/valid_v_l2', valid_v_l2_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/train_conf', train_conf_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}/valid_conf', valid_conf_loss, epoch_num)
        ...
        if epoch_num == 1:
            ...
            save_rgb_datas(true_gt_rgb_layout_imgs[:, CSUA_LDM_VIS_BAND_ORDER, :, :].cpu(), 
                           nrow=6, 
                           savepath=os.path.join(CSUA_LDM_DIFFUSION_NODISENT_RGB_DIR, f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}_True_RGB_0.jpg'),
                           use_sqrt=True)
            if save_tif_imgs:
                save_tif_datas(true_gt_imgs.cpu(), 
                            savepath=os.path.join(CSUA_LDM_DIFFUSION_NODISENT_TIF_DIR, f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}_True_TIF_0.tif'),
        ...
        if is_save or epoch_num % 25 == 0:
            ...
            save_model(
                CSUA_LDM_DIFFUSION_NODISENT_MODEL_SAVEPATH + '.tmp',
                epoch_num,
                csua_diffusion_model,
                optimizer,
                scheduler,
            ...
            os.replace(CSUA_LDM_DIFFUSION_NODISENT_MODEL_SAVEPATH + '.tmp', CSUA_LDM_DIFFUSION_NODISENT_MODEL_SAVEPATH)
        ...
        if epoch_num in EXTRA_MODEL_SAVE_EPOCH_NUM:
            ...
            save_model(
                CSUA_LDM_DIFFUSION_NODISENT_MODEL_SAVEPATH + f'_{epoch_num}e.tmp',
                epoch_num,
                csua_diffusion_model,
                optimizer,
                scheduler,
            ...
            os.replace(
                CSUA_LDM_DIFFUSION_NODISENT_MODEL_SAVEPATH + f'_{epoch_num}e.tmp', 
                CSUA_LDM_DIFFUSION_NODISENT_MODEL_SAVEPATH.replace('.pth', '') + f'_{epoch_num}e.pth'
            )
        ...
        if epoch_num % 5 == 0:
            ...
            hr_down_up_sr_step_imgs, hr_down_up_sr_metric_vals, hr_down_up_sr_step_uncertainty = evaluator.evaluate(
            ...
            hr_down_up_sr_psnr_text = ", ".join(
                [f"{display_name}_PSNR: {hr_down_up_sr_metric_vals[img_name]['psnr']:.6f}" for img_name, display_name in zip(step_order, right_chain_labels)]
            )
            hr_down_up_sr_ssim_text = ", ".join(
                [f"{display_name}_SSIM: {hr_down_up_sr_metric_vals[img_name]['ssim']:.6f}" for img_name, display_name in zip(step_order, right_chain_labels)]
            )
            hr_down_up_sr_gmsd_text = ", ".join(
                [
                    f"{display_name}_GMSD(RGB/NIR): "
                    f"{format_metric_value(hr_down_up_sr_metric_vals[img_name]['gmsd_rgb'])}/"
                    f"{format_metric_value(hr_down_up_sr_metric_vals[img_name]['gmsd_nir'])}"
            ...
            hr_down_up_sr_fsim_text = ", ".join(
                [
                    f"{display_name}_FSIM(RGB/NIR): "
                    f"{format_metric_value(hr_down_up_sr_metric_vals[img_name]['fsim_rgb'])}/"
                    f"{format_metric_value(hr_down_up_sr_metric_vals[img_name]['fsim_nir'])}"
                    for img_name, display_name in zip(step_order, right_chain_labels)
                ]
            )
            hr_down_up_sr_sam_text = ", ".join(
                [f"{display_name}_SAM: {hr_down_up_sr_metric_vals[img_name]['sam']:.6f}" for img_name, display_name in zip(step_order, right_chain_labels)]
            )
            hr_down_up_sr_lpips_text = ", ".join(
                [
                    f"{display_name}_LPIPS_RGB: {format_metric_value(hr_down_up_sr_metric_vals[img_name]['lpips_rgb'])}, "
                    f"{display_name}_LPIPS_NIR: {format_metric_value(hr_down_up_sr_metric_vals[img_name]['lpips_nir'])}"
                    for img_name, display_name in zip(step_order, right_chain_labels)
                ]
            )
            ...
            hr_down_up_sr_quality_summary_text = " | ".join(
                [
                    f"{display_name}[{format_common_metric_summary(hr_down_up_sr_metric_vals[img_name])}]"
                    for img_name, display_name in zip(step_order, right_chain_labels)
                ]
            )
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR ===>> {sr_psnr_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR ===>> {sr_ssim_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR ===>> {sr_gmsd_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR ===>> {sr_fsim_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR ===>> {sr_sam_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR ===>> {sr_lpips_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR Summary ===>> {sr_quality_summary_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR ===>> {hr_down_up_sr_psnr_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR ===>> {hr_down_up_sr_ssim_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR ===>> {hr_down_up_sr_gmsd_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR ===>> {hr_down_up_sr_fsim_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR ===>> {hr_down_up_sr_sam_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR ===>> {hr_down_up_sr_lpips_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR Summary ===>> {hr_down_up_sr_quality_summary_text}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, Left Path Row Order ===>> {", ".join(left_chain_labels)}')
            print(f'    Epoch: {epoch_num}, Iter: {global_iteration}, Right Path Row Order ===>> {", ".join(right_chain_labels)}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR ===>> {sr_psnr_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR ===>> {sr_ssim_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR ===>> {sr_gmsd_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR ===>> {sr_fsim_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR ===>> {sr_sam_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR ===>> {sr_lpips_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, ONE_STEP_SR Summary ===>> {sr_quality_summary_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR ===>> {hr_down_up_sr_psnr_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR ===>> {hr_down_up_sr_ssim_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR ===>> {hr_down_up_sr_gmsd_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR ===>> {hr_down_up_sr_fsim_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR ===>> {hr_down_up_sr_sam_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR ===>> {hr_down_up_sr_lpips_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, HR_DOWN_UP_SR Summary ===>> {hr_down_up_sr_quality_summary_text}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, Left Path Row Order ===>> {", ".join(left_chain_labels)}')
            log_info(log_path=info_path, text=f'>>> Epoch: {epoch_num}, Iter: {global_iteration}, Right Path Row Order ===>> {", ".join(right_chain_labels)}')
            log_info(log_path=info_path, text='')
            metric_key_by_tag = {
                "psnr": "psnr",
                "ssim": "ssim",
            ...
            for tag_name, metric_key in metric_key_by_tag.items():
                for img_name in step_order:
                    writer.add_scalar(
                        f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}_one_step_sr_{tag_name}/{img_name}',
                        step_metric_vals[img_name][metric_key],
                        epoch_num,
                    )
                for img_name, display_name in zip(step_order, right_chain_labels):
                    writer.add_scalar(
                        f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}_hr_down_up_sr_{tag_name}/{display_name}',
                        hr_down_up_sr_metric_vals[img_name][metric_key],
                        epoch_num,
                    )
            ...
            if epoch_num % 5 == 0:
                save_rgb_datas(
                    ...
                    nrow=6,
                    savepath=os.path.join(CSUA_LDM_DIFFUSION_NODISENT_RGB_DIR, f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}_RGB_{epoch_num}.jpg'),
                    is_showminmax=True,
                    use_sqrt=True,
                )
            ...
            for uncertainty_name, file_prefix in uncertainty_save_specs:
                ...
                save_rgb_datas(
                    ...
                    savepath=os.path.join(
                        CSUA_LDM_DIFFUSION_NODISENT_UNC_RGB_DIR,
                        f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}_{file_prefix}_Uncertainty_RGB_{epoch_num}.jpg',
                    ),
                    is_showminmax=True,
                )
                log_info(
                    log_path=info_path,
                    text=(
                        f"    Epoch: {epoch_num}, Iter: {global_iteration}, Saved {uncertainty_name} RGB uncertainty image, "
                ...
                if epoch_num % 25 == 0 and save_tif_imgs:
                    ...
                    save_tif_datas(
                        uncertainty_all.cpu(),
                        savepath=os.path.join(
                            CSUA_LDM_DIFFUSION_NODISENT_UNC_TIF_DIR,
                            f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}_{file_prefix}_Uncertainty_AllCH_{epoch_num}.tif',
                        ),
                    ...
                    log_info(
                        log_path=info_path,
                        text=(
                            f"    Epoch: {epoch_num}, Iter: {global_iteration}, Saved {uncertainty_name} all-channel uncertainty TIF, "
            ...
            if epoch_num % 25 == 0 and save_tif_imgs:
                save_tif_datas(sr_fulllevel_result_imgs.cpu(), 
                               savepath=os.path.join(CSUA_LDM_DIFFUSION_NODISENT_TIF_DIR, f'{CSUA_LDM_DIFFUSION_NODISENT_TAG}_TIF_{epoch_num}.tif'),
                               img_sources_num=len(step_order),
                               geotransforms=draw_geos,
                               projections=draw_projs,
                               is_showminmax=True)
            ...
            sr_step_imgs = None
            sr_step_uncertainty = None
            sr_step_denorm_imgs = None
            sr_fulllevel_result_imgs = None
            uncertainty_rgb = None
            uncertainty_rgb_vis = None
