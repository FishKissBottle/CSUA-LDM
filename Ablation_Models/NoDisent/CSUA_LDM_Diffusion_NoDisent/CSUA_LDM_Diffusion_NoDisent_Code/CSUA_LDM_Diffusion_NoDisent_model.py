...
from CSUA_LDM_NoDisent_Config import (
    ...
    CSUA_LDM_DIFFUSION_NODISENT_STEPS,
    CSUA_LDM_DIFFUSION_NODISENT_COND_SCALE,
    CSUA_LDM_DIFFUSION_NODISENT_UNC_SCALE,
)
from CSUA_LDM_Diffusion_NoDisent_block import (
    CondBlock,
    Downsample,
    Normalize,
    ResnetBlock,
...
class CSUA_LDM_Diffusion_NoDisent_UNet(nn.Module):
    """DDPM UNet for the NoDisent ablation (w/o disentangled latent design)."""
    def __init__(
        self, 
        ch=96, 
        ...
        unc_scale=CSUA_LDM_DIFFUSION_NODISENT_UNC_SCALE,
...
if __name__ == "__main__":
    ...
    imgs_e = torch.rand((3, CSUA_LDM_LATENT_HIDDENCHANNEL, 56, 56)).to(DEVICE)
    imgs_c = torch.rand((3, CSUA_LDM_LATENT_HIDDENCHANNEL, 56, 56)).to(DEVICE)
    imgs_unc = torch.rand((3, CSUA_LDM_LATENT_HIDDENCHANNEL, 56, 56)).to(DEVICE) * 0.5

    print('imgs_e.shape: ', imgs_e.shape)

    diffusion_model = CSUA_LDM_Diffusion_NoDisent_UNet().to(DEVICE)

    t = torch.randint(
        max(int(CSUA_LDM_DIFFUSION_NODISENT_STEPS), 2), size=(imgs_e.shape[0],), device=DEVICE
    )
