...
from CSUA_LDM_NoDisent_Config import (
    ...
    CSUA_LDM_VAE_NODISENT_DRAW_MICROBATCH_SIZE,
    hr_ch_mean,
    hr_ch_std,
)
...
def build_vae_model(
    ...
    device: torch.device,
    load_ema: bool = True,
) -> tuple[LatentAutoEncoderNoDisent, bool, int]:
...
def encode_images_to_latent(
    ...
    """Encode images and return full latent mu and logvar."""
    ...
    return (
        torch.cat(z_chunks, dim=0),
        torch.cat(logvar_chunks, dim=0),
    )
...
def decode_latents_with_uncertainty(
    ...
    logvar_in = None if logvar is None else ensure_batched_4d(logvar).detach().to(device=device)
    ...
    for idx_chunk in iter_microbatch_indices(all_indices, draw_microbatch_size):
        ...
        if logvar_in is not None:
            logvar_chunk = logvar_in.index_select(0, idx_chunk).contiguous().to(dtype=vae_dtype)
        else:
            logvar_chunk = torch.zeros_like(latents_chunk)
        ...
        if use_propagator:
            ...
            # NoDisent: treat entire latent as content, style is empty for propagator interface
            content = latents_chunk
            style = torch.empty(
                latents_chunk.shape[0], 0, *latents_chunk.shape[2:],
                device=latents_chunk.device, dtype=latents_chunk.dtype
            )
            logvar_c = logvar_chunk
            logvar_s = torch.empty(
                logvar_chunk.shape[0], 0, *logvar_chunk.shape[2:],
                device=logvar_chunk.device, dtype=logvar_chunk.dtype
            )
            ...
            pred_mean, pred_var, _, _ = model.decode(latents_chunk, logvar_chunk)
...
def reproject_images_to_latent_mu(
    ...
    _, mu_latent, logvar = model.reparameterize(z_params)
    reproj_mean, reproj_pixel_var, _, _ = model.decode(mu_latent, logvar)

    reproj_params = model.encode(reproj_mean)
    _, reproj_latent, reproj_logvar = model.reparameterize(reproj_params)
    return reproj_mean, reproj_pixel_var, reproj_latent, reproj_logvar
...
def generate_stage_reconstruction_outputs(
    ...
    for stage_name in stage_cache:
        ...
        stage_latent, stage_logvar = encode_images_to_latent(
            stage_norm,
            model,
            draw_microbatch_size=draw_microbatch_size,
            device=device,
        )
        latent_var = torch.exp(stage_logvar)
