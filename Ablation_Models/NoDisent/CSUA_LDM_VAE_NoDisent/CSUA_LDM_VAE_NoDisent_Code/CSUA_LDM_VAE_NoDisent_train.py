...
def ensure_output_dirs():
    os.makedirs(os.path.dirname(CSUA_LDM_VAE_NODISENT_SAVEPATH), exist_ok=True)
    os.makedirs(CSUA_LDM_VAE_NODISENT_LOG_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_VAE_NODISENT_RGB_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_VAE_NODISENT_TIF_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_VAE_NODISENT_UNC_RGB_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_VAE_NODISENT_UNC_TIF_DIR, exist_ok=True)
...
def _decode_compute_losses(model, z, logvar, target_imgs, use_amp):
    """Decode with given full latent and compute reconstruction losses (mean reduction)."""
    with torch.amp.autocast("cuda", enabled=use_amp, dtype=torch.bfloat16):
        pred_mean, pred_var, pred_logvar, raw_var = model.decode(z, logvar)
...
def train(
    ...
    model,
    optimizer,
    gnll_weight: float = CSUA_LDM_VAE_NODISENT_GNLL_WEIGHT,
    kl_weight: float = CSUA_LDM_VAE_NODISENT_KL_WEIGHT,
    l1_weight: float = CSUA_LDM_VAE_NODISENT_L1_WEIGHT,
    l2_weight: float = CSUA_LDM_VAE_NODISENT_L2_WEIGHT,
    confidence_weight: float = CSUA_LDM_VAE_NODISENT_CONFIDENCE_WEIGHT,
    grad_clip_max_norm: float | None = CSUA_LDM_GRAD_CLIP_MAX_NORM,
    ema_model=None,
    ema_decay: float = CSUA_LDM_EMA_DECAY,
    train_microbatch_size: int = CSUA_LDM_VAE_NODISENT_TRAIN_MICROBATCH_SIZE,
    ...
    with tqdm(dataloader, leave=False, colour="#ff924a", ncols=130) as pbar:
        for batch in pbar:
            ...
            for mb_start, mb_end in iter_microbatch_slices(batch_size, train_microbatch_size):
                ...
                gnll, l1, l2, conf = _decode_compute_losses(model, z, logvar, target_img, use_amp)
                ...
                del self_kl_backward, self_kl_loss, z, mu, logvar, gnll, l1, l2, conf, k, kl_tensor

                microbatch_gnll = log_self_gnll
                microbatch_l1 = log_self_l1
                microbatch_l2 = log_self_l2
                microbatch_conf = log_self_conf
...
def valid(
    ...
    eval_microbatch_size: int = CSUA_LDM_VAE_NODISENT_VALID_MICROBATCH_SIZE,
):
    ...
    try:
        for batch in dataloader:
            ...
            for mb_start, mb_end in iter_microbatch_slices(batch_size, eval_microbatch_size):
                ...
                g1 = _decode_compute_losses(model, lr_up_z, lr_up_logvar, lr_up, use_amp)
                g2 = _decode_compute_losses(model, hr_down_up_z, hr_down_up_logvar, hr_down_up, use_amp)
                g3 = _decode_compute_losses(model, hr_z, hr_logvar, hr, use_amp)

                batch_gnll = (g1[0] + g2[0] + g3[0]) / 3.0
                batch_l1 = (g1[1] + g2[1] + g3[1]) / 3.0
                batch_l2 = (g1[2] + g2[2] + g3[2]) / 3.0
                batch_conf = (g1[3] + g2[3] + g3[3]) / 3.0
...
def main(
    ...
    train_microbatch_size: int = CSUA_LDM_VAE_NODISENT_TRAIN_MICROBATCH_SIZE,
    eval_microbatch_size: int = CSUA_LDM_VAE_NODISENT_VALID_MICROBATCH_SIZE,
    draw_microbatch_size: int = CSUA_LDM_VAE_NODISENT_DRAW_MICROBATCH_SIZE
):
    """Entry point for NoDisent VAE."""
    set_random_seed(RANDOM_SEED)
    ensure_output_dirs()
    writer = SummaryWriter(CSUA_LDM_VAE_NODISENT_LOG_DIR)
    ...
    vae_model_is_exist = os.path.exists(VAE_NODISENT_CHECKPOINT_PATH)
    ...
    for epoch_idx in range(start_epoch, TRAIN_MAX_EPOCHS):
        ...
        info_path = CSUA_LDM_VAE_NODISENT_SAVEPATH.replace(".pth", "_TrainingInfo.txt")
        log_info(
            log_path=info_path,
            text=(
                f">>> Epoch: {epoch_num}, Iter: {global_iteration}, Pc: {pc}, Pc_min_lr: {pc_min_lr}, Lr: {current_lr:.6f}, "
        ...
        writer.add_scalars(f'{CSUA_LDM_VAE_NODISENT_TAG}/train_loss', {"train_loss": train_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_NODISENT_TAG}/train_gnll_loss', {"train_gnll_loss": train_gnll_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_NODISENT_TAG}/train_l1_loss', {"train_l1_loss": train_l1_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_NODISENT_TAG}/train_l2_loss', {"train_l2_loss": train_l2_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_NODISENT_TAG}/train_kl_loss', {"train_kl_loss": train_kl_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_NODISENT_TAG}/train_conf_loss', {"train_conf_loss": train_conf_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_NODISENT_TAG}/valid_loss', {"valid_loss": valid_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_NODISENT_TAG}/valid_gnll_loss', {"valid_gnll_loss": valid_gnll_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_NODISENT_TAG}/valid_l1_loss', {"valid_l1_loss": valid_l1_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_NODISENT_TAG}/valid_l2_loss', {"valid_l2_loss": valid_l2_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_NODISENT_TAG}/valid_kl_loss', {"valid_kl_loss": valid_kl_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_NODISENT_TAG}/valid_conf_loss', {"valid_conf_loss": valid_conf_loss}, epoch_num)
        ...
        if epoch_num == 1:
            ...
            save_rgb_datas(
                ...
                nrow=3,
                savepath=os.path.join(CSUA_LDM_VAE_NODISENT_RGB_DIR, f'{CSUA_LDM_VAE_NODISENT_TAG}_True_RGB_0.jpg'),
                is_showminmax=True,
                use_sqrt=True
            )
            if save_tif_imgs:
                save_tif_datas(
                    draw_imgs.cpu(),
                    savepath=os.path.join(CSUA_LDM_VAE_NODISENT_TIF_DIR, f'{CSUA_LDM_VAE_NODISENT_TAG}_True_AllCH_0.tif'),
        ...
        if is_save or epoch_num % 25 == 0:
            ...
            save_model(
                CSUA_LDM_VAE_NODISENT_SAVEPATH + '.tmp',
                epoch_num,
                vae_model,
                optimizer,
                scheduler,
            ...
            os.replace(CSUA_LDM_VAE_NODISENT_SAVEPATH + '.tmp', CSUA_LDM_VAE_NODISENT_SAVEPATH)
        ...
        if epoch_num in EXTRA_MODEL_SAVE_EPOCH_NUM:
            ...
            save_model(
                CSUA_LDM_VAE_NODISENT_SAVEPATH + f'_{epoch_num}e.tmp',
                epoch_num,
                vae_model,
                optimizer,
                scheduler,
            ...
            os.replace(
                CSUA_LDM_VAE_NODISENT_SAVEPATH + f'_{epoch_num}e.tmp',
                CSUA_LDM_VAE_NODISENT_SAVEPATH.replace('.pth', '') + f'_{epoch_num}e.pth'
            )
        ...
        if epoch_num % 5 == 0:
            ...
            info_log_path = CSUA_LDM_VAE_NODISENT_SAVEPATH.replace('.pth', '_TrainingInfo.txt')
            ...
            if draw_metrics_list:
                ...
                for key, value in agg_draw_metrics.items():
                    writer.add_scalar(f'{CSUA_LDM_VAE_NODISENT_TAG}/draw_{key}', value, epoch_num)

            recon_imgs = torch.cat(recon_imgs_list, dim=0)
            save_rgb_datas(
                recon_imgs[:, CSUA_LDM_VIS_BAND_ORDER, :, :].cpu(),
                nrow=3,
                savepath=os.path.join(CSUA_LDM_VAE_NODISENT_RGB_DIR, f'{CSUA_LDM_VAE_NODISENT_TAG}_RGB_{epoch_num}.jpg'),
                is_showminmax=True,
                use_sqrt=True,
            )
            ...
            save_rgb_datas(
                ...
                nrow=3,
                savepath=os.path.join(CSUA_LDM_VAE_NODISENT_UNC_RGB_DIR, f'{CSUA_LDM_VAE_NODISENT_TAG}_Uncertainty_RGB_{epoch_num}.jpg'),
                is_showminmax=True,
                use_sqrt=True,
            )
            ...
            if epoch_num % 25 == 0 and save_tif_imgs:
                save_tif_datas(
                    recon_imgs.cpu(),
                    savepath=os.path.join(CSUA_LDM_VAE_NODISENT_TIF_DIR, f'{CSUA_LDM_VAE_NODISENT_TAG}_Recon_AllCH_{epoch_num}.tif'),
                    img_sources_num=3,
                    geotransforms=draw_geos,
                    projections=draw_projs,
                    is_showminmax=True
                ...
                save_tif_datas(
                    uncertainty_all.cpu(),
                    savepath=os.path.join(CSUA_LDM_VAE_NODISENT_UNC_TIF_DIR, f'{CSUA_LDM_VAE_NODISENT_TAG}_Uncertainty_AllCH_{epoch_num}.tif'),
                    img_sources_num=3,
                    geotransforms=draw_geos,
                    projections=draw_projs,
                    is_showminmax=True
