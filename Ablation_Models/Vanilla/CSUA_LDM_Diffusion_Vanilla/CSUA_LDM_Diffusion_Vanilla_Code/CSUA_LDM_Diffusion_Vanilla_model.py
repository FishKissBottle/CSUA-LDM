...
from CSUA_LDM_Vanilla_Config import (
    ...
    CSUA_LDM_DIFFUSION_VANILLA_STEPS,
    CSUA_LDM_DIFFUSION_VANILLA_COND_SCALE,
)
from CSUA_LDM_Diffusion_Vanilla_block import (
    CondBlock,
    Downsample,
    Normalize,
    ResnetBlock,
...
class CSUA_LDM_Diffusion_Vanilla_UNet(nn.Module):
    """DDPM UNet for the Vanilla ablation (w/o uncertainty and w/o disentangled latent design)."""
    def __init__(
        self, 
        ch=96, 
...
if __name__ == "__main__":
    ...
    imgs_e = torch.rand((3, CSUA_LDM_LATENT_HIDDENCHANNEL, 64, 64)).to(DEVICE)
    imgs_c = torch.rand((3, CSUA_LDM_LATENT_HIDDENCHANNEL, 64, 64)).to(DEVICE)

    print('imgs_e.shape: ', imgs_e.shape)

    diffusion_model = CSUA_LDM_Diffusion_Vanilla_UNet().to(DEVICE)

    t = torch.randint(
        max(int(CSUA_LDM_DIFFUSION_VANILLA_STEPS), 2), size=(imgs_e.shape[0],), device=DEVICE
    )
