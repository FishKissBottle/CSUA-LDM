...
def _dual_branch_lr_up_probability(epoch_idx: int) -> float:
    if not bool(CSUA_LDM_DIFFUSION_VANILLA_DUAL_BRANCH_ENABLED):
        return 1.0
    epoch_idx = int(max(epoch_idx, 0))
    if epoch_idx < int(CSUA_LDM_DIFFUSION_VANILLA_DUAL_BRANCH_WARMUP_EPOCHS):
        return 0.0
    ramp_epochs = max(int(CSUA_LDM_DIFFUSION_VANILLA_DUAL_BRANCH_RAMP_EPOCHS), 1)
    ramp_progress = min(
        max(
            (epoch_idx + 1 - int(CSUA_LDM_DIFFUSION_VANILLA_DUAL_BRANCH_WARMUP_EPOCHS)) / float(ramp_epochs),
            0.0,
        ),
        1.0,
    )
    return float(max(0.0, min(1.0, float(CSUA_LDM_DIFFUSION_VANILLA_DUAL_BRANCH_MAX_LR_UP_PROB)))) * ramp_progress


def select_dual_branch_source(epoch_idx: int | None) -> str:
    if epoch_idx is None or (not bool(CSUA_LDM_DIFFUSION_VANILLA_DUAL_BRANCH_ENABLED)):
        return "lr_up"
