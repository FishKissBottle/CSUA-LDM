...
from CSUA_LDM_Vanilla_Config import *  # type: ignore[reportMissingImports]
...
from CSUA_LDM_VAE_Vanilla_helpers import reproject_images_to_latent_mu
from CSUA_LDM_VAE_Vanilla_model import LatentAutoEncoderVanilla

gdal.UseExceptions()

VANILLA_LOSS_METRIC_KEYS = (
    "total",
    "v_l1",
    "v_l2",
)
...
class DiffusionLossCalculator(nn.Module):
    """Standard DDPM v-pred loss calculator for the Vanilla ablation."""

    def __init__(
        self,
        model,
        betas: torch.Tensor,
        lambda_l1: float = CSUA_LDM_DIFFUSION_VANILLA_V_L1_WEIGHT,
        lambda_l2: float = CSUA_LDM_DIFFUSION_VANILLA_V_L2_WEIGHT,
...
def _encode_image_to_latent(
    ...
    with torch.amp.autocast("cuda", enabled=False):
        ...
        z, mu, _ = vae_model.reparameterize(z_params)
    if bool(use_mu):
        return mu
    return z


def _decode_latent_to_mean(
    vae_model: LatentAutoEncoderVanilla,
    z: torch.Tensor,
) -> torch.Tensor:
    vae_dtype = _get_module_param_dtype(vae_model)
    ...
    with torch.amp.autocast("cuda", enabled=torch.cuda.is_available(), dtype=torch.bfloat16):
        pred_mean = vae_model.decode(z_in)
    return pred_mean.to(dtype=z.dtype)
...
def _compute_loss_for_step(
    ...
    Latent_model: LatentAutoEncoderVanilla,
    ...
    with torch.no_grad():
        if use_reproj_source:
            _, cond_z = reproject_images_to_latent_mu(
                source_imgs,
                Latent_model,
                draw_microbatch_size=CSUA_LDM_DIFFUSION_VANILLA_DRAW_MICROBATCH_SIZE,
                device=device,
            )
        else:
            cond_z = _encode_image_to_latent(Latent_model, source_imgs)
        target_z = _encode_image_to_latent(Latent_model, target_imgs)
...
class OneStepSREvaluator:
    """Evaluate one-step SR for the Vanilla ablation (w/o uncertainty and w/o disentangled latent design)."""
    ...
    def __init__(
        ...
        hr_std: torch.Tensor,
        draw_microbatch_size: int = CSUA_LDM_DIFFUSION_VANILLA_DRAW_MICROBATCH_SIZE,
    ):
    ...
    def _process_microbatch(
        ...
        if use_reproj_condition:
            lr_up_recon_mean, cond_z = reproject_images_to_latent_mu(
                lr_up_chunk,
                self.vae_model,
            ...
            cond_z = _encode_image_to_latent(self.vae_model, lr_up_chunk)
            lr_up_recon_mean = self.vae_model.decode(cond_z)
        ...
        prev_target_z = _encode_image_to_latent(self.vae_model, prev_target_imgs)
        prev_target_mu = _encode_image_to_latent(self.vae_model, prev_target_imgs, use_mu=True)

        sr_prev_imgs, _ = CSUA_LDM_Diffusion_Vanilla_Denoise(
            y0_e=cond_z,
...
def train(data_loader, Latent_model, optimizer, loss_calculator,
         l1_weight: float = CSUA_LDM_DIFFUSION_VANILLA_V_L1_WEIGHT,
         l2_weight: float = CSUA_LDM_DIFFUSION_VANILLA_V_L2_WEIGHT,
         grad_clip_max_norm: float | None = CSUA_LDM_GRAD_CLIP_MAX_NORM,
          epoch_idx: int = 0,
          ema_model=None,
          ema_decay=0.995,
          train_microbatch_size: int = CSUA_LDM_DIFFUSION_VANILLA_TRAIN_MICROBATCH_SIZE,
          start_global_iteration: int = 0,
          max_iterations: int = 0,):
    """
    Train loop for CSUA_LDM diffusion (Vanilla: w/o uncertainty and w/o disentangled latent design).
    ...
    metric_sums = _init_metric_sums(VANILLA_LOSS_METRIC_KEYS)
    num_samples = 0
    ...
    for batch in pbar:
        ...
        batch_metric_sums = _init_metric_sums(VANILLA_LOSS_METRIC_KEYS)
        skip_batch = False
        loss_terms = None
        weighted_loss = None
        loss = None
        ...
        for mb_start, mb_end in iter_microbatch_slices(batch_size, train_microbatch_size):
            ...
            for key in VANILLA_LOSS_METRIC_KEYS:
...
def valid(data_loader, Latent_model, loss_calculator,
         ...
         epoch_idx: int = 0,
         eval_microbatch_size: int = CSUA_LDM_DIFFUSION_VANILLA_VALID_MICROBATCH_SIZE):
...
def ensure_output_dirs():
    os.makedirs(os.path.dirname(CSUA_LDM_DIFFUSION_VANILLA_MODEL_SAVEPATH), exist_ok=True)
    os.makedirs(CSUA_LDM_DIFFUSION_VANILLA_LOG_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_DIFFUSION_VANILLA_RGB_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_DIFFUSION_VANILLA_TIF_DIR, exist_ok=True)
...
def main(dataset_dict: dict = CSUA_LDM_DATASET_DIRS,
    ...
    writer = SummaryWriter(CSUA_LDM_DIFFUSION_VANILLA_LOG_DIR)
    ...
    # ---------- safety ----------
    DIFFUSION_VANILLA_CHECKPOINT_PATH = CSUA_LDM_DIFFUSION_VANILLA_MODEL_SAVEPATH
    is_exist = os.path.exists(DIFFUSION_VANILLA_CHECKPOINT_PATH)
    if is_exist and not resume_train:
        raise Exception(f"{CSUA_LDM_DIFFUSION_VANILLA_TAG} checkpoint exists and resume_train is False.")

    # ---------- build diffusion model ----------
    csua_diffusion_model = CSUA_LDM_Diffusion_Vanilla_UNet(
    ...
    betas = cosine_beta_scheduler(
        timesteps=CSUA_LDM_DIFFUSION_VANILLA_STEPS,
        device=DEVICE,
        dtype=torch.float32,
    )
    loss_calculator = DiffusionLossCalculator(
    ...
    if resume_train:
        csua_diffusion_model, optimizer, scheduler, last_epoch, last_loss_dict, ckpt_extra, loaded_ema_decay = load_model(
            DIFFUSION_VANILLA_CHECKPOINT_PATH,
            csua_diffusion_model,
            optimizer,
            scheduler,
            ema_model=ema_model,
    ...
    print(
        f"[{CSUA_LDM_DIFFUSION_VANILLA_TAG}] VRAM-friendly batch sizes => "
        f"train={CSUA_LDM_TRAIN_BATCH_SIZE}, valid={CSUA_LDM_VALID_BATCH_SIZE}, draw={CSUA_LDM_DRAW_BATCH_SIZE}"
    )
    print(
        f"[{CSUA_LDM_DIFFUSION_VANILLA_TAG}] Internal microbatch sizes => "
        f"train={train_microbatch_size}, valid={eval_microbatch_size}, draw={draw_microbatch_size}"
    )
    print(
        f"[{CSUA_LDM_DIFFUSION_VANILLA_TAG}] Dataset splits => "
        f"train={train_split_label}, valid={valid_split_label}, draw={draw_split_label}"
    )
    print(f"[{CSUA_LDM_DIFFUSION_VANILLA_TAG}] Using fixed one-step diffusion path: cond -> hr")

    # ---------- load latent VAE (once) ----------
    VAE_VANILLA_CHECKPOINT_PATH = CSUA_LDM_VAE_VANILLA_SAVEPATH
    if not os.path.exists(VAE_VANILLA_CHECKPOINT_PATH):
        raise Exception(f"{VAE_VANILLA_CHECKPOINT_PATH} does not exist")
    ...
    for epoch_idx in range(start_epoch, TRAIN_MAX_EPOCHS):
        ...
        info_path = CSUA_LDM_DIFFUSION_VANILLA_MODEL_SAVEPATH.replace(".pth", "_TrainingInfo.txt")
        log_info(
            log_path=info_path,
        ...
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}/train_loss', train_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}/valid_loss',  valid_loss,  epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}/train_l1', train_v_l1_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}/valid_l1', valid_v_l1_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}/train_l2', train_v_l2_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}/valid_l2', valid_v_l2_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}/train_v_l1', train_v_l1_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}/valid_v_l1', valid_v_l1_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}/train_v_l2', train_v_l2_loss, epoch_num)
        writer.add_scalar(f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}/valid_v_l2', valid_v_l2_loss, epoch_num)
        ...
        if epoch_num == 1:
            ...
            save_rgb_datas(true_gt_rgb_layout_imgs[:, CSUA_LDM_VIS_BAND_ORDER, :, :].cpu(), 
                           nrow=6, 
                           savepath=os.path.join(CSUA_LDM_DIFFUSION_VANILLA_RGB_DIR, f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}_True_RGB_0.jpg'),
                           use_sqrt=True)
            if save_tif_imgs:
                save_tif_datas(true_gt_imgs.cpu(), 
                            savepath=os.path.join(CSUA_LDM_DIFFUSION_VANILLA_TIF_DIR, f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}_True_TIF_0.tif'),
        ...
        if is_save or epoch_num % 25 == 0:
            ...
            save_model(
                CSUA_LDM_DIFFUSION_VANILLA_MODEL_SAVEPATH + '.tmp',
                epoch_num,
                csua_diffusion_model,
                optimizer,
                scheduler,
            ...
            os.replace(CSUA_LDM_DIFFUSION_VANILLA_MODEL_SAVEPATH + '.tmp', CSUA_LDM_DIFFUSION_VANILLA_MODEL_SAVEPATH)
        ...
        if epoch_num in EXTRA_MODEL_SAVE_EPOCH_NUM:
            ...
            save_model(
                CSUA_LDM_DIFFUSION_VANILLA_MODEL_SAVEPATH + f'_{epoch_num}e.tmp',
                epoch_num,
                csua_diffusion_model,
                optimizer,
                scheduler,
            ...
            os.replace(
                CSUA_LDM_DIFFUSION_VANILLA_MODEL_SAVEPATH + f'_{epoch_num}e.tmp', 
                CSUA_LDM_DIFFUSION_VANILLA_MODEL_SAVEPATH.replace('.pth', '') + f'_{epoch_num}e.pth'
            )
        ...
        if epoch_num % 5 == 0:
            ...
            hr_down_up_sr_step_imgs, hr_down_up_sr_metric_vals = evaluator.evaluate(
            ...
            hr_down_up_sr_lpips_text = ", ".join(
                [
                    f"{display_name}_LPIPS_RGB: {format_metric_value(hr_down_up_sr_metric_vals[img_name]['lpips_rgb'])}, {display_name}_LPIPS_NIR: {format_metric_value(hr_down_up_sr_metric_vals[img_name]['lpips_nir'])}"
                    for img_name, display_name in zip(step_order, right_chain_labels)
                ]
            )
            ...
            for tag_name, metric_key in metric_key_by_tag.items():
                for img_name in step_order:
                    writer.add_scalar(
                        f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}_one_step_sr_{tag_name}/{img_name}',
                        step_metric_vals[img_name][metric_key],
                        epoch_num,
                    )
                for img_name, display_name in zip(step_order, right_chain_labels):
                    writer.add_scalar(
                        f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}_hr_down_up_sr_{tag_name}/{display_name}',
            ...
            if epoch_num % 5 == 0:
                save_rgb_datas(
                    ...
                    nrow=6,
                    savepath=os.path.join(CSUA_LDM_DIFFUSION_VANILLA_RGB_DIR, f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}_RGB_{epoch_num}.jpg'),
                    is_showminmax=True,
                    use_sqrt=True,
                )
            ...
            if epoch_num % 25 == 0 and save_tif_imgs:
                save_tif_datas(sr_fulllevel_result_imgs.cpu(), 
                               savepath=os.path.join(CSUA_LDM_DIFFUSION_VANILLA_TIF_DIR, f'{CSUA_LDM_DIFFUSION_VANILLA_TAG}_TIF_{epoch_num}.tif'),
