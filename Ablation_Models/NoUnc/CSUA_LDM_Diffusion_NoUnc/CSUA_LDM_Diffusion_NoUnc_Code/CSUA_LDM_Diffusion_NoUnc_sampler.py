...
from CSUA_LDM_NoUnc_Config import (
    CSUA_LDM_DIFFUSION_NOUNC_SAMPLING_NOISE_SCALE,
)
from CSUA_LDM_Diffusion_NoUnc_helpers import extract


class DDPMSampler(nn.Module):
    """Strict v-pred DDPM sampler for the NoUnc variant."""
    ...
    def _predict_v(self, x_t, t, y0):
        ...
        if pred_v_mean is None:
            raise KeyError("Diffusion model must return `pred_v_mean` for NoUnc sampling.")
    ...
    def forward(
        ...
        sampling_noise_scale = float(max(0.0, min(1.0, CSUA_LDM_DIFFUSION_NOUNC_SAMPLING_NOISE_SCALE)))
