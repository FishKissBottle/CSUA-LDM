...
import CSUA_LDM_NoUnc_Config as CSUA_LDM_NoUnc_Config_module
...
DEFAULT_VAE_CKPT = Path(CSUA_LDM_VAE_NOUNC_SAVEPATH)
DEFAULT_ROOT_DIR = Path(CSUA_LDM_DATASET_DIRS["rootdir_list_forTrain"])
VARIANT_YAML_PATH = PROJECT_ROOT / "configs" / "variants" / "nounc.yaml"
...
def write_latent_scaling_factor_to_config(latent_scaling_factor, yaml_path: Path | str = VARIANT_YAML_PATH):
    ...
    dataset_name = str(getattr(CSUA_LDM_NoUnc_Config_module._cfg.dataset, "name", "unknown")).lower()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=f"Compute HR content-only latent scaling factor for {CSUA_LDM_VAE_NOUNC_TAG}."
    )
...
def compute_latent_scaling_factor(
    ...
    with torch.no_grad():
        with tqdm(loader, total=total_batches, ncols=120, desc=f"Compute {CSUA_LDM_VAE_NOUNC_TAG} scaling factor") as pbar:
            for batch_idx, batch in enumerate(pbar):
                ...
                if not (isinstance(rep_out, (list, tuple)) and len(rep_out) >= 6):
                    raise TypeError(
                        "NoUnc VAE `reparameterize()` is expected to return "
    ...
    print(f"\n{CSUA_LDM_VAE_NOUNC_TAG} Content-only latent scaling diagnostics")
