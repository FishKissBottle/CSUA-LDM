...
CSUA_LDM_VAE_NOUNC_ENABLE_MICROBATCH_ENCODING_CACHE = True
...
_VARIANT_YAML_PATH = str(PROJECT_ROOT / "configs" / "variants" / "nounc.yaml")
...
# Microbatch sizes
CSUA_LDM_VAE_NOUNC_TRAIN_MICROBATCH_SIZE = int(_cfg.microbatch_sizes.vae.train)
CSUA_LDM_VAE_NOUNC_VALID_MICROBATCH_SIZE = int(_cfg.microbatch_sizes.vae.valid)
CSUA_LDM_VAE_NOUNC_DRAW_MICROBATCH_SIZE = int(_cfg.microbatch_sizes.vae.draw)
CSUA_LDM_DIFFUSION_NOUNC_TRAIN_MICROBATCH_SIZE = int(_cfg.microbatch_sizes.diffusion.train)
CSUA_LDM_DIFFUSION_NOUNC_VALID_MICROBATCH_SIZE = int(_cfg.microbatch_sizes.diffusion.valid)
CSUA_LDM_DIFFUSION_NOUNC_DRAW_MICROBATCH_SIZE = int(_cfg.microbatch_sizes.diffusion.draw)
...
if LATENT_SCALING_FACTOR <= 0.0:
    raise ValueError(
        ...
        f"It must be > 0. Check configs/variants/nounc.yaml."
    )

# ---------------------------------------------------------------------------
# Variant-specific configuration (NoUnc: w/o uncertainty)
# ---------------------------------------------------------------------------

# VAE losses
CSUA_LDM_VAE_NOUNC_CONTENT_KL_WEIGHT = float(_cfg.vae.losses.content_kl_weight)
CSUA_LDM_VAE_NOUNC_STYLE_KL_WEIGHT = float(_cfg.vae.losses.style_kl_weight)
CSUA_LDM_VAE_NOUNC_L1_WEIGHT = float(_cfg.vae.losses.l1_weight)
CSUA_LDM_VAE_NOUNC_L2_WEIGHT = float(_cfg.vae.losses.l2_weight)

# VAE disentanglement
CSUA_LDM_VAE_NOUNC_CONTENT_CHANNELS = int(_cfg.vae.disentanglement.content_channels)
CSUA_LDM_VAE_NOUNC_STYLE_CHANNELS = int(_cfg.vae.disentanglement.style_channels)
CSUA_LDM_VAE_NOUNC_CONTENT_ALIGN_WEIGHT = float(_cfg.vae.disentanglement.content_align_weight)
CSUA_LDM_VAE_NOUNC_STYLE_ALIGN_WEIGHT = float(_cfg.vae.disentanglement.style_align_weight)
CSUA_LDM_VAE_NOUNC_CONTENT_VARIANCE_WEIGHT = float(_cfg.vae.disentanglement.content_variance_weight)
CSUA_LDM_VAE_NOUNC_CONTENT_VARIANCE_TARGET_STD = float(_cfg.vae.disentanglement.content_variance_target_std)
CSUA_LDM_VAE_NOUNC_STYLE_DROPOUT_RATE = float(_cfg.vae.disentanglement.style_dropout_rate)

# Diffusion conditioning
CSUA_LDM_DIFFUSION_NOUNC_COND_SCALE = float(_cfg.diffusion.conditioning.cond_scale)

# Diffusion losses
CSUA_LDM_DIFFUSION_NOUNC_V_L2_WEIGHT = float(_cfg.diffusion.losses.v_l2_weight)
CSUA_LDM_DIFFUSION_NOUNC_V_L1_WEIGHT = float(_cfg.diffusion.losses.v_l1_weight)

# Diffusion sampling
CSUA_LDM_DIFFUSION_NOUNC_STEPS = int(_cfg.diffusion.sampling.steps)
CSUA_LDM_DIFFUSION_NOUNC_SAMPLING_NOISE_SCALE = float(_cfg.diffusion.sampling.sampling_noise_scale)

# Diffusion dual branch
CSUA_LDM_DIFFUSION_NOUNC_DUAL_BRANCH_ENABLED = bool(_cfg.diffusion.dual_branch.enabled)
CSUA_LDM_DIFFUSION_NOUNC_DUAL_BRANCH_WARMUP_EPOCHS = int(_cfg.diffusion.dual_branch.warmup_epochs)
CSUA_LDM_DIFFUSION_NOUNC_DUAL_BRANCH_RAMP_EPOCHS = int(_cfg.diffusion.dual_branch.ramp_epochs)
CSUA_LDM_DIFFUSION_NOUNC_DUAL_BRANCH_MAX_LR_UP_PROB = float(_cfg.diffusion.dual_branch.max_lr_up_prob)

# Diffusion UNet
CSUA_LDM_DIFFUSION_NOUNC_UNET_CH = int(_cfg.diffusion.unet.ch)
CSUA_LDM_DIFFUSION_NOUNC_UNET_CH_MULT = tuple(_cfg.diffusion.unet.ch_mult)
CSUA_LDM_DIFFUSION_NOUNC_UNET_ATTN_RESOLUTIONS = list(_cfg.diffusion.unet.attn_resolutions)
CSUA_LDM_DIFFUSION_NOUNC_UNET_DROPOUT = float(_cfg.diffusion.unet.dropout)
CSUA_LDM_DIFFUSION_NOUNC_UNET_RESAMP_WITH_CONV = bool(_cfg.diffusion.unet.resamp_with_conv)
CSUA_LDM_DIFFUSION_NOUNC_UNET_RESOLUTION = CSUA_LDM_HR_CROP_SIZE // 8
CSUA_LDM_DIFFUSION_NOUNC_WINDOW_ATTN_MIN_RESOLUTION = int(_cfg.diffusion.unet.window_attn_min_resolution)
CSUA_LDM_DIFFUSION_NOUNC_ATTN_BASE_HEAD_DIM = int(_cfg.diffusion.unet.attn_base_head_dim)
CSUA_LDM_DIFFUSION_NOUNC_ATTN_HEADS_CAP = int(_cfg.diffusion.unet.attn_heads_cap)
...
CSUA_LDM_VAE_NOUNC_EXPERIMENT_ID = _build_experiment_id("CSUA_LDM_VAE", "NoUnc", CSUA_LDM_DATASET_NAME)
CSUA_LDM_DIFFUSION_NOUNC_EXPERIMENT_ID = _build_experiment_id("CSUA_LDM_Diffusion", "NoUnc", CSUA_LDM_DATASET_NAME)

# ---------------------------------------------------------------------------
# Project outputs based on experiment_id
# ---------------------------------------------------------------------------

CSUA_LDM_VAE_NOUNC_LOG_DIR = str(
    VARIANT_ROOT / "CSUA_LDM_VAE_NoUnc" / "CSUA_LDM_VAE_NoUnc_Logs" / CSUA_LDM_VAE_NOUNC_EXPERIMENT_ID
)
CSUA_LDM_VAE_NOUNC_RGB_DIR = str(
    VARIANT_ROOT / "CSUA_LDM_VAE_NoUnc" / "CSUA_LDM_VAE_NoUnc_RGBs" / CSUA_LDM_VAE_NOUNC_EXPERIMENT_ID
)
CSUA_LDM_VAE_NOUNC_TIF_DIR = str(
    VARIANT_ROOT / "CSUA_LDM_VAE_NoUnc" / "CSUA_LDM_VAE_NoUnc_TIFs" / CSUA_LDM_VAE_NOUNC_EXPERIMENT_ID
)
CSUA_LDM_VAE_NOUNC_SAVEPATH = str(
    VARIANT_ROOT
    / "CSUA_LDM_VAE_NoUnc"
    / "CSUA_LDM_VAE_NoUnc_Models_SaveFolder"
    / f"{CSUA_LDM_VAE_NOUNC_EXPERIMENT_ID}.pth"
)
CSUA_LDM_VAE_NOUNC_TAG = CSUA_LDM_VAE_NOUNC_EXPERIMENT_ID

CSUA_LDM_DIFFUSION_NOUNC_LOG_DIR = str(
    VARIANT_ROOT
    / "CSUA_LDM_Diffusion_NoUnc"
    / "CSUA_LDM_Diffusion_NoUnc_Logs"
    / CSUA_LDM_DIFFUSION_NOUNC_EXPERIMENT_ID
)
CSUA_LDM_DIFFUSION_NOUNC_RGB_DIR = str(
    VARIANT_ROOT
    / "CSUA_LDM_Diffusion_NoUnc"
    / "CSUA_LDM_Diffusion_NoUnc_RGBs"
    / CSUA_LDM_DIFFUSION_NOUNC_EXPERIMENT_ID
)
CSUA_LDM_DIFFUSION_NOUNC_TIF_DIR = str(
    VARIANT_ROOT
    / "CSUA_LDM_Diffusion_NoUnc"
    / "CSUA_LDM_Diffusion_NoUnc_TIFs"
    / CSUA_LDM_DIFFUSION_NOUNC_EXPERIMENT_ID
)
CSUA_LDM_DIFFUSION_NOUNC_MODEL_SAVEPATH = str(
    VARIANT_ROOT
    / "CSUA_LDM_Diffusion_NoUnc"
    / "CSUA_LDM_Diffusion_NoUnc_Models_SaveFolder"
    / f"{CSUA_LDM_DIFFUSION_NOUNC_EXPERIMENT_ID}.pth"
)
CSUA_LDM_DIFFUSION_NOUNC_TAG = CSUA_LDM_DIFFUSION_NOUNC_EXPERIMENT_ID
