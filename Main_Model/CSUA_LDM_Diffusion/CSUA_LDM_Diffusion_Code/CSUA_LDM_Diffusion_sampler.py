...
from Main_Model.CSUA_LDM_Config import (
    CSUA_LDM_DIFFUSION_INFERENCE_UNCERT_GUIDANCE_ENABLED,
    CSUA_LDM_DIFFUSION_INFERENCE_UNCERT_GUIDANCE_NOISE_GATE_MAX,
    CSUA_LDM_DIFFUSION_INFERENCE_UNCERT_GUIDANCE_NOISE_GATE_MIN,
    CSUA_LDM_DIFFUSION_INFERENCE_UNCERT_GUIDANCE_RELIABILITY_ALPHA,
    CSUA_LDM_DIFFUSION_INFERENCE_UNCERT_GUIDANCE_SAVE_MAPS,
    CSUA_LDM_DIFFUSION_SAMPLING_NOISE_SCALE,
    CSUA_LDM_DIFFUSION_UNC_COND_LOGVAR_SOFTEN_LAMBDA,
)
from CSUA_LDM_Diffusion_helpers import extract
...
class DDPMSampler(nn.Module):
    ...
    def __init__(
        ...
        self.enable_uncertainty_guidance = bool(CSUA_LDM_DIFFUSION_INFERENCE_UNCERT_GUIDANCE_ENABLED)
        self.guidance_reliability_alpha = float(CSUA_LDM_DIFFUSION_INFERENCE_UNCERT_GUIDANCE_RELIABILITY_ALPHA)
        self.guidance_noise_gate_min = float(CSUA_LDM_DIFFUSION_INFERENCE_UNCERT_GUIDANCE_NOISE_GATE_MIN)
        self.guidance_noise_gate_max = float(CSUA_LDM_DIFFUSION_INFERENCE_UNCERT_GUIDANCE_NOISE_GATE_MAX)
        self.guidance_save_maps = bool(CSUA_LDM_DIFFUSION_INFERENCE_UNCERT_GUIDANCE_SAVE_MAPS)
    ...
    def _to_ptr_tensor(self, ptrs, bsz: int, device: torch.device) -> torch.Tensor:
    ...
    def forward(
        ...
        if start_step is None:
            ...
            start_step = int(max(0, min(int(start_step), self.T - 1)))

        total_steps = int(start_step) + 1
        ...
        sampling_noise_scale = float(max(0.0, min(1.0, CSUA_LDM_DIFFUSION_SAMPLING_NOISE_SCALE)))
        ...
        for step_idx, time_step in enumerate(pbar):
            ...
            if time_step == 0:
                ...
                final_info = {
                    ...
                    "start_step": torch.tensor(int(start_step), device=x_t.device, dtype=torch.long),
                }
