...
try:
    from Main_Model.CSUA_LDM_VAE.CSUA_LDM_VAE_Code.CSUA_LDM_VAE_block import DownBlock, MidBlock, UpBlock
except ModuleNotFoundError:
    from CSUA_LDM_VAE_block import DownBlock, MidBlock, UpBlock


class LatentAutoEncoder(nn.Module):
    """VAE with explicit Content/Style latent disentanglement.
    ...
    External interface exposes 8-channel latent I/O.
    """
    ...
    def __init__(
        ...
        self.logvar_content_proj = nn.Conv2d(self.content_channels, self.down_channels[-1], kernel_size=3, padding=1)
        self.logvar_style_proj = nn.Conv2d(self.style_channels, self.down_channels[-1], kernel_size=3, padding=1)
    ...
    def decode(self, 
        ...
        logvar_feat = self.logvar_content_proj(logvar_content) + self.logvar_style_proj(logvar_style)
    ...
    def forward(self, x: torch.Tensor):
        ...
        z_content, z_style, _, _, logvar_content, logvar_style = self.reparameterize(z_params)
        pred_mean, pred_var, pred_logvar, raw_var = self.decode(
            z_content, z_style, logvar_content, logvar_style
        )
