...
for search_path in (PROJECT_ROOT, MODULE_DIR):
    search_path_str = str(search_path)
    if search_path_str not in sys.path:
        sys.path.insert(0, search_path_str)

import Main_Model.CSUA_LDM_Config as CSUA_LDM_Config_module
...
DEFAULT_VAE_CKPT = Path(CSUA_LDM_VAE_SAVEPATH)
DEFAULT_ROOT_DIR = Path(CSUA_LDM_DATASET_DIRS["rootdir_list_forTrain"])
VARIANT_YAML_PATH = PROJECT_ROOT / "configs" / "variants" / "main.yaml"
...
def write_latent_scaling_factor_to_config(latent_scaling_factor, yaml_path: Path | str = VARIANT_YAML_PATH):
    ...
    dataset_name = str(getattr(CSUA_LDM_Config_module._cfg.dataset, "name", "unknown")).lower()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=f"Compute HR content-only latent scaling factor for {CSUA_LDM_VAE_TAG}."
    )
...
def compute_latent_scaling_factor(
    ...
    with torch.no_grad():
        with tqdm(loader, total=total_batches, ncols=120, desc=f"Compute {CSUA_LDM_VAE_TAG} scaling factor") as pbar:
            for batch_idx, batch in enumerate(pbar):
                ...
                if not (isinstance(rep_out, (list, tuple)) and len(rep_out) >= 6):
                    raise TypeError(
                        "Main VAE `reparameterize()` is expected to return "
                        "(z_content, z_style, mu_content, mu_style, logvar_content, logvar_style)."
                    )
                z_content = rep_out[0]
                mu_content = rep_out[2]
    ...
    payload = {
        ...
        "latent_source": "encode(...) -> reparameterize(...)[2] (mu_content only)",
    ...
    print(f"\n{CSUA_LDM_VAE_TAG} Content-only latent scaling diagnostics")
