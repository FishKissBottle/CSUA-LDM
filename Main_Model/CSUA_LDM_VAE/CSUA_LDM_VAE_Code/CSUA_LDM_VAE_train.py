...
for _search_path in (REPO_ROOT, MODULE_DIR):
...
from CSUA_LDM_VAE_helpers import (
    build_stage_cache_from_batch,
    decode_latents_with_uncertainty,
    denorm_mean,
    denorm_std,
    norm_img,
...
def ensure_output_dirs():
    os.makedirs(os.path.dirname(CSUA_LDM_VAE_SAVEPATH), exist_ok=True)
    os.makedirs(CSUA_LDM_VAE_LOG_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_VAE_RGB_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_VAE_TIF_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_VAE_UNC_RGB_DIR, exist_ok=True)
    os.makedirs(CSUA_LDM_VAE_UNC_TIF_DIR, exist_ok=True)
...
def _decode_compute_losses(model, z_content, z_style, logvar_content, logvar_style, target_imgs, use_amp):
    """Decode with given content/style and compute reconstruction losses (mean reduction)."""
    if CSUA_LDM_VAE_STYLE_DROPOUT_RATE > 0.0 and model.training:
        drop_mask = (
            torch.rand((z_style.shape[0], 1, 1, 1), device=z_style.device)
            < CSUA_LDM_VAE_STYLE_DROPOUT_RATE
        )
        if bool(drop_mask.any().item()):
            z_style = torch.where(drop_mask, torch.zeros_like(z_style), z_style)
            logvar_style = torch.where(drop_mask, torch.zeros_like(logvar_style), logvar_style)
    with torch.amp.autocast("cuda", enabled=use_amp, dtype=torch.bfloat16):
        pred_mean, pred_var, pred_logvar, raw_var = model.decode(z_content, z_style, logvar_content, logvar_style)
        gnll = F.gaussian_nll_loss(
            input=pred_mean,
            target=target_imgs,
            var=pred_var,
            full=True,
...
def train(
    ...
    model,
    optimizer,
    gnll_weight: float = CSUA_LDM_VAE_GNLL_WEIGHT,
    content_kl_weight: float = CSUA_LDM_VAE_CONTENT_KL_WEIGHT,
    style_kl_weight: float = CSUA_LDM_VAE_STYLE_KL_WEIGHT,
    l1_weight: float = CSUA_LDM_VAE_L1_WEIGHT,
    l2_weight: float = CSUA_LDM_VAE_L2_WEIGHT,
    confidence_weight: float = CSUA_LDM_VAE_CONFIDENCE_WEIGHT,
    grad_clip_max_norm: float | None = CSUA_LDM_GRAD_CLIP_MAX_NORM,
    content_align_weight: float = CSUA_LDM_VAE_CONTENT_ALIGN_WEIGHT,
    style_align_weight: float = CSUA_LDM_VAE_STYLE_ALIGN_WEIGHT,
    content_variance_weight: float = CSUA_LDM_VAE_CONTENT_VARIANCE_WEIGHT,
    content_variance_target_std: float = CSUA_LDM_VAE_CONTENT_VARIANCE_TARGET_STD,
    ema_model=None,
    ema_decay: float = CSUA_LDM_EMA_DECAY,
    train_microbatch_size: int = CSUA_LDM_VAE_TRAIN_MICROBATCH_SIZE,
    start_global_iteration: int = 0,
    max_iterations: int = 0,
    enable_cache: bool = CSUA_LDM_VAE_ENABLE_MICROBATCH_ENCODING_CACHE,
):
    ...
    with tqdm(dataloader, leave=False, colour="#ff924a", ncols=130) as pbar:
        for batch in pbar:
            ...
            for mb_start, mb_end in iter_microbatch_slices(batch_size, train_microbatch_size):
                ...
                gnll, l1, l2, conf = _decode_compute_losses(
                    model, zc, zs, lvc, lvs, target_img, use_amp
                )
                ...
                self_kl_loss = (
                    float(gnll_weight) * gnll
                    + float(l1_weight) * l1
                    + float(l2_weight) * l2
                    + float(confidence_weight) * conf
                    + kl_tensor
                )
                log_self_gnll = float(gnll.detach().cpu().item())
                log_self_l1 = float(l1.detach().cpu().item())
                log_self_l2 = float(l2.detach().cpu().item())
                log_self_conf = float(conf.detach().cpu().item())
                ...
                if enable_cache:
                    del self_kl_backward, self_kl_loss, zc, zs, mc, ms, lvc, lvs, gnll, l1, l2, conf, kl_c, kl_s, kl_tensor
                else:
                    del self_kl_backward, self_kl_loss, zc, zs, mc, ms, lvc, lvs, gnll, l1, l2, conf, kl_c, kl_s, kl_tensor
                ...
                if cross_branch_name == "g4":
                    ...
                    zc, _, mc1, _, lvc, _ = model.reparameterize(z_params1)
                    z_params2 = _get_cached_encoding(enc_cache, "hr_down_up", model, hr_down_up, use_amp)
                    _, zs, mc2, _, _, lvs = model.reparameterize(z_params2)
                    gnll, l1, l2, conf = _decode_compute_losses(
                        model, zc, zs, lvc, lvs, hr_down_up, use_amp
                    )
                    ...
                    zc, _, _, ms1, lvc, _ = model.reparameterize(z_params1)
                    z_params2 = _get_cached_encoding(enc_cache, "hr_down_up", model, hr_down_up, use_amp)
                    _, zs, _, ms2, _, lvs = model.reparameterize(z_params2)
                    gnll, l1, l2, conf = _decode_compute_losses(
                        model, zc, zs, lvc, lvs, hr, use_amp
                    )
                ...
                log_cross_gnll = float(gnll.detach().cpu().item())
                log_cross_l1 = float(l1.detach().cpu().item())
                log_cross_l2 = float(l2.detach().cpu().item())
                log_cross_conf = float(conf.detach().cpu().item())
                ...
                del cross_align_backward, cross_align_loss, zc, zs, lvc, lvs, gnll, l1, l2, conf, align_tensor
                ...
                batch_gnll_loss_sum += microbatch_gnll * microbatch_size
                ...
                batch_conf_loss_sum += microbatch_conf * microbatch_size
            ...
            batch_gnll = batch_gnll_loss_sum / float(batch_size)
            ...
            batch_conf = batch_conf_loss_sum / float(batch_size)
            ...
            pbar.set_postfix(
                ordered_dict={
                    # "Recon": f"{self_branch_name}+{cross_branch_name}",
                    # "KLsel": kl_branch_name,
                    # "Align": align_branch_name,
                    "Loss": f"{batch_loss:>4.5f}",
                    "GNLL": f"{batch_gnll:>4.3f}",
                    "L1": f"{batch_l1:>4.3f}",
                    "L2": f"{batch_l2:>4.3f}",
                    "KL": f"{batch_kl:>4.3f}",
                    "Conf": f"{batch_conf:>4.3f}",
                    "CA": f"{ca_val:>4.3f}",
                    "SA": f"{sa_val:>4.3f}",
                    "CV": f"{cv_val:>4.3f}",
                }
...
def valid(
    ...
    content_variance_weight: float = CSUA_LDM_VAE_CONTENT_VARIANCE_WEIGHT,
    content_variance_target_std: float = CSUA_LDM_VAE_CONTENT_VARIANCE_TARGET_STD,
    eval_microbatch_size: int = CSUA_LDM_VAE_VALID_MICROBATCH_SIZE,
):
    ...
    try:
        for batch in dataloader:
            ...
            for mb_start, mb_end in iter_microbatch_slices(batch_size, eval_microbatch_size):
                ...
                g1 = _decode_compute_losses(model, lr_up_z_c, lr_up_z_s, lr_up_logvar_c, lr_up_logvar_s, lr_up, use_amp)
                g2 = _decode_compute_losses(model, hr_down_up_z_c, hr_down_up_z_s, hr_down_up_logvar_c, hr_down_up_logvar_s, hr_down_up, use_amp)
                g3 = _decode_compute_losses(model, hr_z_c, hr_z_s, hr_logvar_c, hr_logvar_s, hr, use_amp)
                g4 = _decode_compute_losses(
                    model, lr_up_z_c, hr_down_up_z_s, lr_up_logvar_c, hr_down_up_logvar_s, hr_down_up, use_amp
                )
                g5 = _decode_compute_losses(model, hr_z_c, hr_down_up_z_s, hr_logvar_c, hr_down_up_logvar_s, hr, use_amp)

                batch_gnll = (g1[0] + g2[0] + g3[0]) / 3.0 + (g4[0] + g5[0]) / 2.0
                batch_l1 = (g1[1] + g2[1] + g3[1]) / 3.0 + (g4[1] + g5[1]) / 2.0
                batch_l2 = (g1[2] + g2[2] + g3[2]) / 3.0 + (g4[2] + g5[2]) / 2.0
                batch_conf = (g1[3] + g2[3] + g3[3]) / 3.0 + (g4[3] + g5[3]) / 2.0
                ...
                total_loss = (
                    float(gnll_weight) * batch_gnll
                    + float(l1_weight) * batch_l1
                    + float(l2_weight) * batch_l2
                    + batch_kl
                    + float(confidence_weight) * batch_conf
                ...
                gnll_loss_sum += float(batch_gnll.detach().float().cpu().item()) * microbatch_size
                ...
                conf_loss_sum += float(batch_conf.detach().float().cpu().item()) * microbatch_size
...
def main(
    ...
    ensure_output_dirs()
    writer = SummaryWriter(CSUA_LDM_VAE_LOG_DIR)
    ...
    vae_model_is_exist = os.path.exists(VAE_CHECKPOINT_PATH)
    ...
    vae_model = LatentAutoEncoder(
        ...
        style_channels=CSUA_LDM_VAE_STYLE_CHANNELS,
    ).to(DEVICE)
    ...
    for epoch_idx in range(start_epoch, TRAIN_MAX_EPOCHS):
        ...
        (
            train_loss, train_gnll_loss, train_l1_loss, train_l2_loss, train_kl_loss,
            train_conf_loss, train_ca_loss, train_sa_loss, train_cv_loss,
        ...
        (
            valid_loss, valid_gnll_loss, valid_l1_loss, valid_l2_loss, valid_kl_loss,
            valid_conf_loss, valid_ca_loss, valid_sa_loss, valid_cv_loss
        ) = valid(
            valid_loader,
            ema_model,
            gnll_weight=gnll_weight,
        ...
        print(
            ...
            f"    GNLL_Loss: {valid_gnll_loss:.6f}, L1_Loss: {valid_l1_loss:.6f}, L2_Loss: {valid_l2_loss:.6f}, "
            f"KL_Loss: {valid_kl_loss:.6f}, Conf_Loss: {valid_conf_loss:.6f}, "
            f"CA_Loss: {valid_ca_loss:.6f}, SA_Loss: {valid_sa_loss:.6f}, CV_Loss: {valid_cv_loss:.6f}, "
            f"PeakVRAM(GB): {peak_vram_gb:.3f}"
        )

        info_path = CSUA_LDM_VAE_SAVEPATH.replace(".pth", "_TrainingInfo.txt")
        log_info(
            log_path=info_path,
            text=(
                f">>> Epoch: {epoch_num}, Iter: {global_iteration}, Pc: {pc}, Pc_min_lr: {pc_min_lr}, Lr: {current_lr:.6f}, "
        ...
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/train_loss', {"train_loss": train_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/train_gnll_loss', {"train_gnll_loss": train_gnll_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/train_l1_loss', {"train_l1_loss": train_l1_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/train_l2_loss', {"train_l2_loss": train_l2_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/train_kl_loss', {"train_kl_loss": train_kl_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/train_conf_loss', {"train_conf_loss": train_conf_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/train_ca_loss', {"train_ca_loss": train_ca_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/train_sa_loss', {"train_sa_loss": train_sa_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/train_cv_loss', {"train_cv_loss": train_cv_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/valid_loss', {"valid_loss": valid_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/valid_gnll_loss', {"valid_gnll_loss": valid_gnll_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/valid_l1_loss', {"valid_l1_loss": valid_l1_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/valid_l2_loss', {"valid_l2_loss": valid_l2_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/valid_kl_loss', {"valid_kl_loss": valid_kl_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/valid_conf_loss', {"valid_conf_loss": valid_conf_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/valid_ca_loss', {"valid_ca_loss": valid_ca_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/valid_sa_loss', {"valid_sa_loss": valid_sa_loss}, epoch_num)
        writer.add_scalars(f'{CSUA_LDM_VAE_TAG}/valid_cv_loss', {"valid_cv_loss": valid_cv_loss}, epoch_num)
        ...
        if epoch_num == 1:
            ...
            save_rgb_datas(
                ...
                nrow=3,
                savepath=os.path.join(CSUA_LDM_VAE_RGB_DIR, f'{CSUA_LDM_VAE_TAG}_True_RGB_0.jpg'),
                is_showminmax=True,
                use_sqrt=True
            )
            if save_tif_imgs:
                save_tif_datas(
                    draw_imgs.cpu(),
                    savepath=os.path.join(CSUA_LDM_VAE_TIF_DIR, f'{CSUA_LDM_VAE_TAG}_True_AllCH_0.tif'),
        ...
        if is_save or epoch_num % 25 == 0:
            ...
            os.replace(CSUA_LDM_VAE_SAVEPATH + '.tmp', CSUA_LDM_VAE_SAVEPATH)
        ...
        if epoch_num in EXTRA_MODEL_SAVE_EPOCH_NUM:
            ...
            save_model(
                CSUA_LDM_VAE_SAVEPATH + f'_{epoch_num}e.tmp',
                epoch_num,
                vae_model,
                optimizer,
                scheduler,
            ...
            os.replace(
                CSUA_LDM_VAE_SAVEPATH + f'_{epoch_num}e.tmp',
                CSUA_LDM_VAE_SAVEPATH.replace('.pth', '') + f'_{epoch_num}e.pth'
            )
        ...
        if epoch_num % 5 == 0:
            ...
            recon_imgs_list = []
            uncertainty_rgb_list = []
            uncertainty_all_list = []
            ...
            for stage_name in ["lr_up", "hr_down_up", "hr"]:
                ...
                recon_pred_std = recon_outputs[stage_name]["pred_std"]
                ...
                std_denorm = denorm_std(recon_pred_std, hr_std_gpu)
                ...
                uncertainty_rgb_list.append(std_denorm[:3, CSUA_LDM_VIS_BAND_ORDER, :, :])
                uncertainty_all_list.append(std_denorm[:3])

            # Style-transfer reconstructions
            content_lr_up, style_lr_up, logvar_lr_up_c, logvar_lr_up_s = encode_images_to_content_style(
                get_stage_img(draw_stage_norm_imgs, "lr_up"), ema_model,
                draw_microbatch_size=draw_microbatch_size, device=DEVICE
            )
            content_hr_down_up, style_hr_down_up, logvar_hr_down_up_c, logvar_hr_down_up_s = encode_images_to_content_style(
                get_stage_img(draw_stage_norm_imgs, "hr_down_up"), ema_model,
                draw_microbatch_size=draw_microbatch_size, device=DEVICE
            )
            content_hr, style_hr, logvar_hr_c, logvar_hr_s = encode_images_to_content_style(
                get_stage_img(draw_stage_norm_imgs, "hr"), ema_model,
                draw_microbatch_size=draw_microbatch_size, device=DEVICE
            )

            # decoder(lr_up content, hr_down_up style)
            mixed_z_1 = torch.cat([content_lr_up, style_hr_down_up], dim=1)
            pred_mean_1, _, _ = decode_latents_with_uncertainty(
                mixed_z_1, ema_model,
                logvar_content=logvar_lr_up_c, logvar_style=logvar_hr_down_up_s,
                draw_microbatch_size=draw_microbatch_size, device=DEVICE
            )
            recon_denorm_1 = denorm_mean(pred_mean_1, hr_mean_gpu, hr_std_gpu)
            target_hr_down_up = denorm_mean(get_stage_img(draw_stage_norm_imgs, "hr_down_up"), hr_mean_gpu, hr_std_gpu)
            metrics_1 = evaluate_quality_metrics(recon_denorm_1, target_hr_down_up)
            metrics_lines.append(format_metric_summary("lr_up_content_hr_down_up_style_vs_hr_down_up", metrics_1))
            draw_metrics_list.append(metrics_1)
            recon_imgs_list.append(recon_denorm_1[:3])

            # decoder(hr content, hr_down_up style)
            mixed_z_2 = torch.cat([content_hr, style_hr_down_up], dim=1)
            pred_mean_2, _, _ = decode_latents_with_uncertainty(
                mixed_z_2, ema_model,
                logvar_content=logvar_hr_c, logvar_style=logvar_hr_down_up_s,
                draw_microbatch_size=draw_microbatch_size, device=DEVICE
            )
            recon_denorm_2 = denorm_mean(pred_mean_2, hr_mean_gpu, hr_std_gpu)
            target_hr = denorm_mean(get_stage_img(draw_stage_norm_imgs, "hr"), hr_mean_gpu, hr_std_gpu)
            metrics_2 = evaluate_quality_metrics(recon_denorm_2, target_hr)
            metrics_lines.append(format_metric_summary("hr_content_hr_down_up_style", metrics_2))
            draw_metrics_list.append(metrics_2)
            recon_imgs_list.append(recon_denorm_2[:3])

            # decoder(hr content, lr_up style)
            mixed_z_3 = torch.cat([content_hr, style_lr_up], dim=1)
            pred_mean_3, _, _ = decode_latents_with_uncertainty(
                mixed_z_3, ema_model,
                logvar_content=logvar_hr_c, logvar_style=logvar_lr_up_s,
                draw_microbatch_size=draw_microbatch_size, device=DEVICE
            )
            recon_denorm_3 = denorm_mean(pred_mean_3, hr_mean_gpu, hr_std_gpu)
            metrics_3 = evaluate_quality_metrics(recon_denorm_3, target_hr)
            metrics_lines.append(format_metric_summary("hr_content_lr_up_style", metrics_3))
            draw_metrics_list.append(metrics_3)
            recon_imgs_list.append(recon_denorm_3[:3])

            print(f"    Epoch: {epoch_num}, ===>> Quality Metrics")
            for metric_line in metrics_lines:
                print(f"    {metric_line}")

            info_log_path = CSUA_LDM_VAE_SAVEPATH.replace('.pth', '_TrainingInfo.txt')
            ...
            if draw_metrics_list:
                ...
                for key, value in agg_draw_metrics.items():
                    writer.add_scalar(f'{CSUA_LDM_VAE_TAG}/draw_{key}', value, epoch_num)

            recon_imgs = torch.cat(recon_imgs_list, dim=0)
            save_rgb_datas(
                recon_imgs[:, CSUA_LDM_VIS_BAND_ORDER, :, :].cpu(),
                nrow=3,
                savepath=os.path.join(CSUA_LDM_VAE_RGB_DIR, f'{CSUA_LDM_VAE_TAG}_RGB_{epoch_num}.jpg'),
                is_showminmax=True,
                use_sqrt=True,
            )

            uncertainty_rgb = torch.cat(uncertainty_rgb_list, dim=0)
            uncertainty_rgb_vis = normalize_uncertainty_for_rgb_vis(uncertainty_rgb)
            save_rgb_datas(
                uncertainty_rgb_vis.cpu(),
                nrow=3,
                savepath=os.path.join(CSUA_LDM_VAE_UNC_RGB_DIR, f'{CSUA_LDM_VAE_TAG}_Uncertainty_RGB_{epoch_num}.jpg'),
                is_showminmax=True,
                use_sqrt=True,
            )
            ...
            if epoch_num % 25 == 0 and save_tif_imgs:
                save_tif_datas(
                    recon_imgs.cpu(),
                    savepath=os.path.join(CSUA_LDM_VAE_TIF_DIR, f'{CSUA_LDM_VAE_TAG}_Recon_AllCH_{epoch_num}.tif'),
                    img_sources_num=6,
                    geotransforms=draw_geos,
                    projections=draw_projs,
                    is_showminmax=True
                ...
                uncertainty_all = torch.cat(uncertainty_all_list, dim=0)
                save_tif_datas(
                    uncertainty_all.cpu(),
                    savepath=os.path.join(CSUA_LDM_VAE_UNC_TIF_DIR, f'{CSUA_LDM_VAE_TAG}_Uncertainty_AllCH_{epoch_num}.tif'),
                    img_sources_num=3,
                    geotransforms=draw_geos,
                    projections=draw_projs,
                    is_showminmax=True
                ...
                log_info(
                    ...
                    text=(
                        f"    Epoch: {epoch_num}, Saved all-channel uncertainty TIF, "
            ...
            uncertainty_rgb_list = None
            uncertainty_all_list = None
            metrics_lines = None
            if torch.cuda.is_available():
                gc.collect()
